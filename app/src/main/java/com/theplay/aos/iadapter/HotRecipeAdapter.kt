...
interface HotRecipeAdapterListener{
    fun DoubleTap(postId : Int)
    fun clickedLike(postId : Int)
}

class HotRecipeAdapter(private val activity : Activity, private val context: Context, private val items: MutableList<HotRecipeDetailResponse.Content>) : RecyclerView.Adapter<HotRecipeAdapter.HotRecipeVH>() {
    var listener : HotRecipeAdapterListener? = null

    fun setInterface(hotRecipeAdapterListener: HotRecipeAdapterListener) {
        this.listener = hotRecipeAdapterListener
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): HotRecipeVH {
        val itemBinding = ItemMainBoardBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return HotRecipeVH(itemBinding)
    }

    override fun getItemCount(): Int = items.size

    override fun onBindViewHolder(holder: HotRecipeVH, position: Int) {
        val item: HotRecipeDetailResponse.Content = items[position]
        holder.bind(item, position)
    }

    inner class HotRecipeVH(var binding: ItemMainBoardBinding) : RecyclerView.ViewHolder(binding.root) {
        var visibleTag = false
        private lateinit var mDetector: GestureDetectorCompat

        @SuppressLint("ClickableViewAccessibility")
        fun bind(item: HotRecipeDetailResponse.Content, position: Int) {
...
//            Log.d(TAG, imageHeight.toString())
            ...
            var tagList : MutableList<HotRecipeDetailResponse.AlcoholTag> = mutableListOf()
            for(tag in item.alcoholTags) {
                tagList.add(tag)
            }
            binding.rvTag.adapter = HotRecipeTagAdapter(activity,context,tagList)
...
//                    activity.findNavController(R.id.main_nav_host_fragment).navigate(HomeFragmentDirections.actionHomeFragmentToMainBoardDetailFragment(position))
    ...
    companion object{
        const val TAG = "HotRecipeAdapter"
    }
}
